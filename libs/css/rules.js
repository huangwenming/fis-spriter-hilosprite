...
var Rules = Object.derive(function (id, css) {
    var self = this
        ...
        , __sprites_re = /([?&])__hilosprite=?([a-z0-9_-]+)?(&)?/i  // 支持分组合并，多参数
        , __sprites_hook_ld = '<<<'
        , __sprites_hook_rd = '>>>';
    //selectors
    self.id = id;
    ...
    //获取spriter的配置
    self._settings = fis.config.get('settings.spriter.hilosprite');
    /**
     * get position
     * @param res
     * @private
    ...
    self.image = css.replace(__sprites_re, function (value, first, group, last) {
        var info;
        self._group = group ? group : '__default__';
        info = _.stringQuote(css);
    ...
    self.image = _.stringQuote(self.image).rest;
    if (self._is_sprites) {
        self._css = __sprites_hook_ld + 'spriteId:' + self.id + __sprites_hook_rd;
    }
    else {
        self._css = __sprites_hook_ld + self._css + __sprites_hook_rd;
    }
    ...
    /*self._css = css.replace(__background_re,
        function(m, image, position, repeat, size) {
            ...
            console.log(__sprites_hook_ld + m + __sprites_hook_rd);
