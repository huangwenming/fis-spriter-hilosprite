...
Generator.prototype = {
    ...
    after: function (group, image, arr_selector, direct, scale, list) {
        ...
        if (this.settings.ie_bug_fix) {
            ...
            for (var i = 0; i < n; i++) {
                var step = i * MAX;
                var selectors = arr_selector.slice(step, step + MAX);
                for(var j = 0; j < selectors.length; j++) {
                    this.css[selectors[j]] = '"' + imageUrl + '", ' + this.css[selectors[j]];
                }
            }
        } else {
            for(var j = 0; j < arr_selector.length; j++) {
                this.css[arr_selector[j]] = '"' + imageUrl + '", ' + this.css[arr_selector[j]];
            }
        }
    ...
    fill: function(group, list, direct) {
        ...
        for (i = 0, len = images.length; i < len; i++) {
            ...
            for (k = 0, count = images[i].cls.length; k < count; k++) {
                // 存储相应id的css样式
                this.css[images[i].cls[k].selector] = 'rect: ['
                + -(images[i].cls[k].position[0] + -x) / rem + this.units + ', '
                + -(images[i].cls[k].position[1] + -y) / rem + this.units + ', '
                + images[i].width / rem + this.units + ', '
                + images[i].height / rem + this.units + ']';
        ...
        if (images[zero]) {
            for (i = 0, length = images[zero].length; i < length; i++) {
                ...
                for (j = 0, count = current.cls.length; j < count; j++) {
                    ...
                    this.css[current.cls[j].selector] = 'rect: ['
                        + -x_ / rem + this.units + ', '
                        + -y_ / rem + this.units + ', '
                        + (current.w - this.settings.margin)* scale / rem + this.units + ', '
                        + (current.h - this.settings.margin) * scale / rem + this.units + ']';
